...
To test this out, a virtual reality experience was developed, where the user has to use their expressiveness to get closer to a virtual agent, who will match the user's expressiveness. The system takes in bodily movement through the VR controllers, as input, and then outputs bodily expressions through the avatar, which the user then perceives and then continues to interact, based on what they perceive in the agent's movement, and then the loop continues with the user then giving new bodily movement input. 
